...
class TestGridValidation:
    ...
    def test_mismatched_2d_params(self):
        ...
        with pytest.raises((ValueError, TypeError)):
            Grid(x_start=0.0, x_end=10.0, num_points_x=32,
                 y_start=0.0, num_points_y=32)
...
class TestParameterCombinations:
    ...
    def test_conflicting_2d_parameters(self):
        ...
        grid = Grid(x_start=0.0, x_end=10.0, num_points_x=32,
                   y_start=0.0, y_end=5.0, num_points_y=16)
        # Different aspect ratio - should work
        ic = InitialCondition(grid, func_type='gaussian', 
                            x0=5.0, y0=2.5, sigma_x=1.0, sigma_y=0.5)
        assert ic.f0 is not None
