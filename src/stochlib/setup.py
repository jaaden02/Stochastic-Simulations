...
class Grid:
    ...
    def __init__(
        ...
        if not np.isfinite(x_start) or not np.isfinite(x_end):
            raise ValueError(
                f"x_start and x_end must be finite, got x_start={x_start}, x_end={x_end}"
            )
        ...
        if num_points_x > MAX_POINTS_PER_AXIS:
            raise ValueError(
                f"num_points_x={num_points_x} exceeds maximum allowed ({MAX_POINTS_PER_AXIS}). "
        ...
        if num_points_y is not None and num_points_y > 0:
            if num_points_y > MAX_POINTS_PER_AXIS:
                raise ValueError(
                    f"num_points_y={num_points_y} exceeds maximum allowed ({MAX_POINTS_PER_AXIS}). "
                    f"This would require excessive memory."
                )
        ...
        if num_points_z is not None and num_points_z > 0:
            if num_points_z > MAX_POINTS_PER_AXIS:
                raise ValueError(
                    f"num_points_z={num_points_z} exceeds maximum allowed ({MAX_POINTS_PER_AXIS}). "
                    f"This would require excessive memory."
                )
        ...
        if total_points > MAX_TOTAL_POINTS:
            raise ValueError(
                f"Total grid points ({total_points:,}) exceeds maximum allowed ({MAX_TOTAL_POINTS:,}). "
...
class InitialCondition:
    ...
    def __init__(
        self, grid: Grid, func_type: str = "gaussian", normalize: bool = True, **params: Any
    ) -> None:
        """
    ...
    def __repr__(self) -> str:
        return (
            f"InitialCondition(type='{self.func_type}', "
...
class DiffusionConfig:
    ...
    def __init__(
        ...
        for axis, value in list(self.constants.items()):
            ...
            if not np.isfinite(value):
                raise ValueError(
                    f"Diffusion coefficient for axis '{axis}' must be finite, got {value}"
                )
    ...
    def build_fields(
        self, t: Optional[float] = None, include_inactive: bool = False, evaluate: bool = True
    ) -> Dict[str, Any]:
...
class VelocitiesConfig:
    ...
    def __init__(
        self,
        grid: Grid,
        mu_x: Optional[Any] = None,
        mu_y: Optional[Any] = None,
        mu_z: Optional[Any] = None,
    ) -> None:
        self.grid: Grid = grid
    ...
    def build_fields(
        self, t: Optional[float] = None, evaluate: bool = True
    ) -> Dict[str, np.ndarray]:
        """
...
class SimulationSetup:
    ...
    def build(self):
        ...
        if mode == "fokker_planck":
            ...
            if any(
                v is None
                for v in [self.grid, self.velocities, self.diffusions, self.boundary_conditions]
            ):
                raise ValueError(
