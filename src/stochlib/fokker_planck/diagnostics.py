...
class StabilityAnalyzer:
    ...
    def analyze(
        ...
        diff_rates: float = sum(
            [(2.0 * np.max(diff_fields[ax])) / (deltas[ax] ** 2)
...
class SolutionDiagnostics:
    ...
    def analyze_step(self, f: np.ndarray, t: float) -> Dict:
        ...
        entropy_change: float = entropy - self.history['entropy'][-2] if len(self.history['entropy']) > 1 else 0.0
