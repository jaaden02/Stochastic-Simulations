...
class FokkerPlanckSolver:
    ...
    def solve_step(
        ...
        if f.ndim == 1:
            ...
            f_work = f
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug("solve_step: original_shape=%s work_shape=%s dt=%.3e", original_shape, f_work.shape, dt)
        ...
        f_new = universal_fp_step_kernel(f_work, MU, D, d_coords, dt, self.bc_flags, self.scheme_flags)
